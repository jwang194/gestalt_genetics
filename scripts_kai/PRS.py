...
for r in R:
    ...
    beta_df = pd.DataFrame({f'BETA_{i}': df['BETA'].values for i, df in enumerate(estimates)})
    r2_list_base = PRS_evaluate_multi(truth_pheno.iloc[:, 2:], genotypes, beta_df)

    beta_df_maxh = pd.DataFrame({f'BETA_{i}': df['BETA'].values for i, df in enumerate(maxh_estimates)})
    r2_list_maxh = PRS_evaluate_multi(truth_pheno_maxh.iloc[:, 2:], genotypes, beta_df_maxh)

    # next: sum PRS 
    # is each maxh trait different?
...
# read in gwas effect sizes (snps x 1)

# PRS score = genotypes @ gwas betas



# read in true phenotype values

# check R2 between true phenotype values and PRS 

...
num_pheno <- 10
#num_pheno <- 5
#heritabilities <- c(0.4, 0.3, 0.15, 0.5, 0.2, 'MaxH', 'SUM_PRS')
heritabilities <- c(rep(0.1, times = num_pheno), 'MaxH', 'SUM_PRS')
...
for (p_current in p_thresholds){
  ...
  df_merged <- data.frame(df_P, df_MG, df_MG_sum)
  long_df <- stack(df_merged[, 1:(num_pheno+2)])
  ...
  ggsave(filename = paste0(setting, "threshold_" ,p_current , "_PRS_withSUM.png"), width = 6, height = 6)
}
...
colnames(df_MG) <- "PRS_MaxH"
colnames(df_MG_sum) <- "PRS_SUM"
df_best <- data.frame(df_P, df_MG, df_MG_sum)
...
long_df <- stack(df_best[, 1:(num_pheno+2)])
colnames(long_df) <- c("Rsquared", "Trait")
...
ggplot(long_df, aes(x = Heritability, y = Rsquared)) + 
  ...
  theme_bw() +
  theme_classic(base_size = 16)
ggsave(filename = paste0(setting, "threshold_best_PRS_withSUM.png"), width = 6, height = 6)

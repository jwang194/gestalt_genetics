...
def main():
    ...
    print("Number of CV folds:", args.cv_folds)
    ...
    ### Start cross validation evaluation ###
    ### Discovery = train GWAS
    ### Validation = tune p-value thresholds or compute coefficients to combine PRS (same validation cohort used for every fold of cross-validation)
    ...
    if len(p_thresholds) > 1 or 'SUMPRS_MG' in models:
        discoverytest_validation_plink2 = plink_validation_split(main_plink2, args.intermediate_dir, args.validation_fraction, "main") 
            # "main" is prefix of _discovery_test.* and _validation.* plink files
        discoverytest_plink2 = discoverytest_validation_plink2[0]
        validation_plink2 = discoverytest_validation_plink2[1]
    else:
        discoverytest_plink2 = main_plink2
        validation_plink2 = None

    # split main plink file into cross validation folds 
    folds = plink_cv_split(discoverytest_plink2, args.intermediate_dir, args.cv_folds) # array of tuples: [(train plink fold, test plink fold)...]

    # PRS section for P phenotypes
    if 'P' in models:
        print("PRS for P model")
        p_thresholds_P, predictions_P = PRS_CV(folds, main_plink2.bfile + '.acount', ldref_plink2, PHENOS['P'], args.intermediate_dir, p_thresholds, validation_plink2)
    ...
    if 'MG' in models:
        ...
        p_thresholds_MG, predictions_MG = PRS_CV(folds, main_plink2.bfile + '.acount', ldref_plink2, PHENOS['MG'], args.intermediate_dir, p_thresholds, validation_plink2)
    ...
    if 'PRSSUM' in models:
        ...
        p_thresholds_PRSSUM, predictions_PRSSUM = PRS_CV(folds, main_plink2.bfile + '.acount', ldref_plink2, PHENOS['SUM'], args.intermediate_dir, p_thresholds, validation_plink2)
    ...
    if 'SUMPRS_MG' in models:
        ...
        validation_folds = [(validation_trainfit_plink2[0], validation_trainfit_plink2[1]),(validation_trainfit_plink2[1], validation_trainfit_plink2[0])] # gwas on one predict on other (vice versa)
        p_thresholds_validation, validation_predictions = PRS_CV(validation_folds, main_plink2.bfile + '.acount', ldref_plink2, PHENOS['P'], args.intermediate_dir, [0.05], None)
        ...
        # --- saving code for when using the best p-value threshold for each phenotype, before fixing error described on line 221
        #validation_predictions = pd.read_csv(validation_plink2.bfile + '.fam', header = None, sep = '\t', usecols=[0, 1]) # collect predictions for each phenotype here 
        #validation_predictions.columns = ['FID', 'IID']
        #for phenotype in list(PHENOS['P'].columns)[2:]: # here we run prs for each phenotype separately because best p-value (which we already found in CV) can be different for each phenotype
            #p_best_current = [p_thresholds_P[(phenotype, 1)]] # use best p-value threshold found in first iteration of CV
            #phenotype_df_current = PHENOS['P'].loc[:,['FID', 'IID', phenotype]]
            #thresholds_current, predictions_current = PRS_CV(validation_folds, main_plink2.bfile + '.acount', ldref_plink2, phenotype_df_current, args.intermediate_dir, p_best_current, None)
            #merged_current = pd.merge(validation_predictions, predictions_current, on = ['FID', 'IID'], how = "inner")
            #validation_predictions = merged_current
        #validation_plink2.filter_phenotype(PHENOS['MG'])
        #validation_MG = pd.read_csv(validation_plink2.pheno, sep = '\t')
        #validation_predictions_reordered = validation_predictions.set_index('IID', drop = False).reindex(validation_MG['IID']).reset_index(drop = True)
        # ---

...
#polygenicity=(0.05)
#shared_fraction=(0.4)
#genetic_covariance=(0.04) # of shared components (total genetic covariance between traits is 1/2 of this value)
#environmental_covariance=(-0.1)
...
iterations=20 # number of independent simulations per setting
    ...
    for shared in "${shared_fraction[@]}"; do
        for cov in "${genetic_covariance[@]}"; do
            for env in "${environmental_covariance[@]}"; do
                ...
                qsub prs_pipeline.sh $N $M $iterations $setting ${genotypes} ${ldref} ${phenotype_dir} ${out_dir} ${PLINK2_PATH}
            done
        done
    done
done
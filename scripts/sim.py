...
def simulate(n,m,alleles,assignments,gg,ge,spikes=None,sample_genotypes=True):
    ...
    subsets = set(assignments)
    betas = np.zeros((m,nphen))
    ...
    for s in subsets:
        ...
        for p in product(s,s):
            ...
            counts[p[0],p[1]] += indices.shape[0]
    # gg /= counts 
    start_time = time.perf_counter()
    for s in subsets:
        indices = np.where(np.array(assignments) == s)[0]
        sub_m = indices.shape[0]
        s = [int(se) for se in s.split(',')]
        sub_p = len(s)
        sub_gg = gg[np.ix_(s,s)]
        tmp_beta = np.zeros((m,sub_p))
        tmp_beta[indices] = matrix_normal(np.zeros((sub_m,sub_p)),np.eye(sub_m),sub_gg/sub_m).rvs(1)
        betas[:,s] += tmp_beta
    betas *= np.sqrt(np.diag(gg)/m)/betas.std(0)
